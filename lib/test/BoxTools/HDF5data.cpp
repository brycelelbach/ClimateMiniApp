...
main(int argc, char* argv[])
{
  ...
  if (icode != 0)
    {
      pout() << indent << pgmname <<" failed"<<std::endl;
    }
  else
    {
...
int test()
{
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "File creation failed "<<error<<std::endl;
      return error;
    }
  ...
  if ( verbose )
  {
    pout() << "plan1: " << procID() << "...." << plan1 << std::endl;
    pout() << "plan2: " << procID() << "...." << plan2 << std::endl;
  }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "box write failed "<<error<<std::endl;
      return error;
    }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "BoxLayoutData 1 write failed "<<error<<std::endl;
      return error;
    }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "box2 write failed "<<error<<std::endl;
      return error;
    }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "BoxLayoutData 2 write failed "<<error<<std::endl;
      return error;
    }
 ...
 CH_assert(!testFile.isOpen());
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "File open failed "<<error<<std::endl;
      return error;
    }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "box read failed "<<error<<std::endl;
      return error;
    }
  boxes.sort();
  Vector<int> assign;
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "BoxLayout LoadBalance failed "<<error<<std::endl;
      return error;
    }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "BoxLayoutData<BaseFab<int>> read failed "<<error<<std::endl;
      return error;
    }
  ...
  if ( verbose )
  {
    pout() << "readplan1: " << procID() << "...." << readplan1 << std::endl;
    pout() << "readplan2: " << procID() << "...." << readplan2 << std::endl;
  }
  ...
  if (level1.boxLayout().size() != readlevel1.boxLayout().size())
    {
      if ( verbose )
        pout() << indent2 << "level1.size() != readl1.size() read failed "<<error<<std::endl;
      return 1;
    }
  if (level2.boxLayout().size() != readlevel2.boxLayout().size())
    {
      if ( verbose )
        pout() << indent2 << "level2.size() != readl2.size() read failed "<<error<<std::endl;
      return 1;
    }
          ...
          for (BoxIterator it(level1.box(l1())); it.ok(); ++it)
            {
              if ((*before)(it(), c) != (*after)(it(), c))
                {
                  if ( verbose )
                    pout() << indent2 << "l1 != readl1 read failed "<<error<<std::endl;
                  return 2;
                }
            }
        }
          ...
          for (BoxIterator it(level2.box(l2())); it.ok(); ++it)
            {
              if ((*before)(it(), c) != (*after)(it(), c))
                {
                  if ( verbose )
                    pout() << indent2 << "level2 != readlevel2 read failed "<<error<<std::endl;
                  return 3;
                }
            }
        }
  ...
  if (error != 0)
    {
      if ( verbose )
        pout() << indent2 << "readLevel failed "<<error<<std::endl;
      return error;
    }
          ...
          for (BoxIterator it(state.box(dit())) ; it.ok() ; ++it)
                {
                  if (readFAB(it(), 0) != fab(it(), 1))
                        {
                          if ( verbose )
                                pout() << indent2 << "state != after for out-of-core "<<error<<std::endl;
                          return 3;
                        }
                }
        }
