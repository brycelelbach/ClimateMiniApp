...
copyTest()
{
  ...
  pout() << "layout = " << loLayoutCoar << std::endl;
  pout() << "copier = " << copierCoarToFine << std::endl;
  ...
  for (ditFine.reset(); ditFine.ok(); ++ditFine)
    {
      ...
      if (Abs(rmax - valcomp) > eps)
        {
          if (verbose)
            pout() << indent2 << "max is wrong" << std::endl;
          icode =-1;
        }
      if (Abs(rmin - valcomp) > eps)
        {
          if (verbose)
            pout() << indent2 << "min is wrong" << std::endl;
          icode =-2;
        }
    }
  if (verbose)
    {
      pout() << indent2 << "LoFabCoar:" << std::endl;
      for (ditCoar.reset(); ditCoar.ok(); ++ditCoar)
        {
          FArrayBox& fab = loFabCoar[ditCoar()];
          ...
          for (bit.reset(); bit.ok(); ++bit)
            {
              pout() << indent2 << bit() << indent2 << fab(bit(),0) << std::endl;
            }
        }
      pout() << indent2 << "LoFabFine:" << std::endl;
      for (ditFine.reset(); ditFine.ok(); ++ditFine)
        {
          FArrayBox& fab = loFabFine[ditFine()];
          BoxIterator bit(fab.box());
