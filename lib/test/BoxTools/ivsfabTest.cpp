...
int testIVS()
{
  ...
  if ( ! ( ivs == ivstest ) )
  {
    if ( verbose ) pout() << "error: " << pgmname << ": failed IntVectSet consistency test" << std::endl;
    ++errors ;
  }
  // test that numIvs() returns the expected result
  if ( fab.numIvs() != box.volume() )
  {
    if ( verbose ) pout() << "error: " << pgmname << ": failed numIvs() test" << std::endl;
    ++errors ;
  }
      ...
      if ( fab(bit(),icomp) != 7.5 )
      {
        if ( verbose )
          pout() << "error: " << pgmname << ": value at (" << bit() << "," << icomp
               << ") is not what setVal() set" << std::endl;
        ++errors;
      }
    }
  }
      ...
      if ( fab(ivsit(), icomp) != val )
      {
        if ( verbose )
          pout() << "error: " << pgmname << ": value at (" << ivsit() << "," << icomp
               << ") is " << fab(ivsit(), icomp) << ") but should be " << val << std::endl ;
        ++errors;
      }
    }
  }
