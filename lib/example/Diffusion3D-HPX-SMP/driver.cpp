...
#include "diffusion_3d.hpp"

template <typename F>
void visit(AsyncLevelData<diffusion_3d::problem_state>& soln, F f)
{
...
void output(
    AsyncLevelData<diffusion_3d::problem_state>& data
...
int chombo_main(variables_map& vm)
{
    ...
    diffusion_3d::configuration config(
    ...
    diffusion_3d::aniso_profile profile(config,
    ...
    AsyncLevelData<diffusion_3d::problem_state> data(dbl, IntVect::Unit);
    DefineData(data, 1);
    ...
    typedef diffusion_3d::imex_operators<diffusion_3d::aniso_profile> imexop;
    typedef AsyncARK4<diffusion_3d::problem_state, imexop> ark_type;
    ...
    if (output_flag)
    {
        AsyncLevelData<diffusion_3d::problem_state> src(dbl, config.ghost_vector);
        DefineData(src, 1);
        visit(src,
            [&profile](Real& val, IntVect here)
            { val = profile.source_term(here); }
        );
        output(src, "source.hdf5", "phi", 0); 
    }
#endif 
    ...
    while (time < config.nt)
    {
        ...
        { 
            ...
            {
                exchanges.push_back(
                    hpx::async(LocalExchangeSync<diffusion_3d::problem_state>
            ...
            output(data, "phi.%06u.hdf5", "phi", step); 
    
            AsyncLevelData<diffusion_3d::problem_state> analytic(dbl, config.ghost_vector);
            DefineData(analytic, 1);
            visit(analytic,
                [&profile, time](Real& val, IntVect here)
                { val = profile.analytic_solution(here, time); }
            );
            output(analytic, "analytic.%06u.hdf5", "phi", step); 
    
            for (dit.begin(); dit.ok(); ++dit)
                analytic[dit()].data.increment(data[dit()].data, -1.0);
            output(analytic, "error.%06u.hdf5", "phi", step); 
        }
#endif 
...
int main(int argc, char** argv)
{
    boost::program_options::options_description cmdline(
        "HPX/Chombo AMR Climate Mini-App (Diffusion Problem)"
        );
